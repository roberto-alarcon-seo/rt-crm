-- Drop old uuid-typed function and recreate with text partner_id
DROP FUNCTION IF EXISTS public.partner_wallet_adjust(uuid, integer, text);
...
REVOKE ALL ON FUNCTION public.partner_wallet_adjust(text, integer, text) FROM public;
GRANT EXECUTE ON FUNCTION public.partner_wallet_adjust(text, integer, text) TO authenticated;