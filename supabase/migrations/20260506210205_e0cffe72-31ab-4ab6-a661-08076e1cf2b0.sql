...
BEGIN
  ...
  ELSE
    ...
    -- Property-scoped round robin: pick the authorized agent with the
    -- LEAST active leads (ties broken by oldest last_assigned_at on contacts).
    IF v_contact.re_property_interest_id IS NOT NULL THEN
      ...
      WHERE pa.property_id = v_contact.re_property_interest_id
        ...
        AND p.status = 'active'
        AND (v_max_cap IS NULL OR public.fn_count_active_leads_for_agent(v_conv.tenant_id, pa.user_id) < v_max_cap)
      ORDER BY
        public.fn_count_active_leads_for_agent(v_conv.tenant_id, pa.user_id) ASC,
        (SELECT COALESCE(MAX(c2.updated_at), 'epoch'::timestamptz)
           FROM public.contacts c2
          WHERE c2.assigned_agent_id = pa.user_id
            AND c2.tenant_id = v_conv.tenant_id) ASC,
        pa.user_id ASC
      LIMIT 1;
    END IF;
  ...
  IF v_strategy IN ('round_robin','property') THEN
