...
BEGIN
  ...
  ON CONFLICT (tenant_id) DO UPDATE SET
    ...
    sticky_agent_enabled = EXCLUDED.sticky_agent_enabled,
    sticky_overrides_property = EXCLUDED.sticky_overrides_property,
    lead_timeout_minutes = EXCLUDED.lead_timeout_minutes,
  ...
  -- Upsert profiles (existing trigger may have already created them)
  INSERT INTO public.profiles (id, tenant_id, name, email, status, is_active_for_assignment)
  VALUES
    (v_admin, v_tenant, 'Test Admin', 'test_admin_'||substr(v_admin::text,1,6)||'@test.local', 'active', true),
    (v_a1,    v_tenant, 'Asesor 1',   'test_a1_'||substr(v_a1::text,1,6)||'@test.local',     'active', true),
    (v_a2,    v_tenant, 'Asesor 2',   'test_a2_'||substr(v_a2::text,1,6)||'@test.local',     'active', true),
    (v_a3,    v_tenant, 'Asesor 3',   'test_a3_'||substr(v_a3::text,1,6)||'@test.local',     'active', true)
  ...
  VALUES
    (v_admin, 'user'::global_role, 'administrador'::tenant_role),
    (v_a1,    'user'::global_role, 'asesor'::tenant_role),
    (v_a2,    'user'::global_role, 'asesor'::tenant_role),
    (v_a3,    'user'::global_role, 'asesor'::tenant_role)
  ...
  v_results := v_results || jsonb_build_object('scenario','1. Round Robin global (sin propiedad, sin sticky)','expected','strategy=round_robin','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id,'success',v_res.success),'passed',(v_res.success AND v_res.strategy='round_robin' AND v_res.agent_id IN (v_a1,v_a2,v_a3,v_admin)));
  ...
  v_results := v_results || jsonb_build_object('scenario','2. Sticky: agente previo se mantiene','expected','strategy=sticky, agent=Asesor 2','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.strategy='sticky' AND v_res.agent_id=v_a2));
  ...
  v_results := v_results || jsonb_build_object('scenario','3. Propiedad con asesor autorizado','expected','strategy=property, agent=Asesor 1','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.strategy='property' AND v_res.agent_id=v_a1));
  ...
  v_results := v_results || jsonb_build_object('scenario','4. Sticky vence a Propiedad cuando override=true','expected','strategy=sticky, agent=Asesor 2','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.strategy='sticky' AND v_res.agent_id=v_a2));
  ...
  SELECT v_tenant, 'load_'||g, '+52155500'||(1000+g)::text, v_a1, 'active'::contact_status, 'ACTIVE' FROM generate_series(1,5) g;
  ...
  v_results := v_results || jsonb_build_object('scenario','5. Propiedad con 2 asesores: gana el de menor carga','expected','agent=Asesor 3 (Asesor 1 saturado)','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.strategy='property' AND v_res.agent_id=v_a3));
  ...
  v_results := v_results || jsonb_build_object('scenario','6. max_active_leads_per_agent excluye saturados','expected','agent=Asesor 3 (Asesor 1 saturado > cap 2)','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.agent_id=v_a3));
  ...
  v_results := v_results || jsonb_build_object('scenario','7. is_active_for_assignment=false excluye al asesor','expected','agent=Asesor 2 o Admin','actual',jsonb_build_object('strategy',v_res.strategy,'agent',v_res.agent_id),'passed',(v_res.agent_id=v_a2 OR v_res.agent_id=v_admin));
  ...
  v_results := v_results || jsonb_build_object('scenario','8. Sin asesores activos: error NO_AGENTS_AVAILABLE','expected','success=false, error_code=NO_AGENTS_AVAILABLE','actual',jsonb_build_object('success',v_res.success,'error_code',v_res.error_code),'passed',(v_res.success=false AND v_res.error_code='NO_AGENTS_AVAILABLE'));
  ...
  END LOOP;
  v_results := v_results || jsonb_build_object('scenario','11. Round Robin rota entre asesores','expected','al menos 2 asesores distintos en 3 asignaciones','actual',jsonb_build_object('agents_seen',to_jsonb(v_agents_seen)),'passed',(SELECT COUNT(DISTINCT a) FROM unnest(v_agents_seen) a) >= 2);
  ...
  v_results := v_results || jsonb_build_object('scenario','12. Timeout monitor marca conversación en riesgo','expected','risk_flagged_at IS NOT NULL','actual',jsonb_build_object('flagged',v_flagged),'passed',v_flagged);
  ...
  BEGIN DELETE FROM public.assignment_logs WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.conversations WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.contacts WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.property_assignments WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.properties WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.assignment_rules WHERE tenant_id = v_tenant; EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.user_roles WHERE user_id IN (v_admin, v_a1, v_a2, v_a3); EXCEPTION WHEN OTHERS THEN NULL; END;
  BEGIN DELETE FROM public.profiles WHERE id IN (v_admin, v_a1, v_a2, v_a3); EXCEPTION WHEN OTHERS THEN NULL; END;
